...
void
file_writer (const char *filename, const char *var,
            int *num1, int *num2)
{
  ...
  buf_page = malloc(sizeof(char) * (f_stat.st_size + 2));
  ...
  if (buf_page != NULL)
  {
    ...
    buf_new_page = malloc(sizeof(char) * (strlen(buf_page) + 2));
  ...
  if (strcmp(method, MHD_HTTP_METHOD_POST) == 0)
  {
    ...
    else
    {
      ...
      if (strcmp(filename, "/result.html") == 0 && con_info->answerstring != NULL)
      {
        ...
        file_writer(filename, NULL, response_num[2], NULL);
        
        return send_page (connection, filename, false);
      }
      else if (strcmp(filename, "/takur.html") == 0 && response_num != NULL)
      {
        file_writer(filename, NULL, response_num[0], response_num[1]);
